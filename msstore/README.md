...
1. ~~Reserve the name.~~ **Done 2026-09-03: `FileHasher - Checksum Utility`.**
   The bare `FileHasher` was not available, which is what the Store already
   carrying **File Hasher** (Aftnet) and **Files Hasher** (Star Studio)
   predicted. Consequences to keep in mind: the Store title now differs from
   the winget and Chocolatey listings, which both say `FileHasher`, so once
   this is live `winget search filehasher` returns two entries whose names do
   not match; and Partner Center shows the product under the first name
   reserved. A second name can be added later from **Product management,
   Manage app names**, and the display name is chosen from the **Product name**
   dropdown on the Store listing page. Do not delete a reserved name to tidy
   up, since it is released for anyone else immediately.
